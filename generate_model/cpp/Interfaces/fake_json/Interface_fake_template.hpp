...
{% if PARENT -%}
#include "Interfaces/{{PARENT.F_NAME}}/{{PARENT.NAME}}_fake_json.hpp"
{%- else -%}
...
class {{NAME}}_fake_json :
     public {{D_NAME}},
     public {%if PARENT %}{{PARENT.D_NAME}}_fake_json{%else%}CompoMe::Fake_json{% endif %} {
public:
  // constructor
  {{NAME}}_fake_json(CompoMe::Function_json_send& out, CompoMe::Return_json_recv& in);

  //! Destructor
  virtual ~{{NAME}}_fake_json() noexcept;
