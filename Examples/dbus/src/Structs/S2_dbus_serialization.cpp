...
S2::to_stream(DBus::MessageAppendIterator &os,
  ...
  export_field(*os.sub_iterator(), p_ctx, "S2", "type", "s");

  export_field(*os.sub_iterator(), p_ctx, this->i1, "i1", "i");

  export_field(*os.sub_iterator(), p_ctx, this->i2, "i2", "i");
...
S2::from_stream(DBus::MessageIterator &is,
  ...
  while (i1.is_valid()) {
    ...
    case str2int("i2"): {
      ...
      import_field(i3, p_ctx, this->i2);

      break;
    }

    default:
      std::cerr << "wrong attribute: \"" << k << "\" not in S2";
      throw "wrong attribute: \"" + k + "\" not in S2";
      break;
    } // end switch
