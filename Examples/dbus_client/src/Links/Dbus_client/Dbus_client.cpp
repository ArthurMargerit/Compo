...
void Dbus_client::connect(Require_helper &t) {
  ...
  t.fake_dbus_it(*fs, fs->get_rs());
  this->function_dbus[&t] = fs;
}
...
string Dbus_client::get_object_name() const { return this->object_name; }

void Dbus_client::set_object_name(const string p_object_name) {
