...
Compute_fac::Compute_fac() { this->init(); }

Compute_fac::~Compute_fac() {}

Compute *Compute_fac::build(const std::string &p_type, std::istream &p_stream,
                            CompoMe::Serialization_context_import &p_ctx) {

  if (p_type == "Compute") {
    // void* l_addr = get_addr(is);
    // if(p_ctx.is_share_ptr(l_addr)) {
    //   std::shared_ptr<{NAME}}> b = std::make_shared<Compute>();
...
std::shared_ptr<Compute> Compute_fac::build_sp(const std::string &p_type,
                                               std::istream &p_stream) {

  if (p_type == "Compute") {
    auto b = std::make_shared<Compute>();
    p_stream >> *b;
    return b;
  }
  ...
  std::cerr << "Error: of std::shared_ptr<Compute> build "
            << "Your type \"" << p_type
            << "\" is not include or not init as a child." << std::endl;
  ...
  CompoMe::Serializable_fac::Build_fac_f f =
      [](const std::string &str, std::istream &p_s,
        ...
        return dynamic_cast<CompoMe::Serializable_Item *>(
            Compute_fac::get_inst().build(str, p_s, l_ctx));
      };
  CompoMe::Serializable_fac::Build_fac_f_sp f_sp = [](const std::string &str,
                                                      std::istream &p_s) {
    return Compute_fac::get_inst().build_sp(str, p_s);
  };

  CompoMe::Serializable_fac::get_inst().subscribe("Compute", f, f_sp);
}

void Compute_fac::subscribe(const std::string &ss, Build_fac_f v,
                            Build_fac_f_sp v_sp) {
