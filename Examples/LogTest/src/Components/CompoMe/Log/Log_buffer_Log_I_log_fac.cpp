...
std::ostream &Log_buffer_Log_I_log::to_stream(
    std::ostream &os, CompoMe::Serialization_context_export &p_ctx) const {
  os << "{";
  os << "type:"
     << "CompoMe::Log::Log_buffer_Log_I_log";

  os << "}";
  return os;
}

std::istream &Log_buffer_Log_I_log::from_stream(
  ...
  do {
    ...
    case str2int("type"): {
      ...
      if (t.first != "CompoMe::Log::Log_buffer_Log_I_log") {
        throw "Wrong Type: ";
      }
      break;
    }
    ...
    default:
      C_ERROR("wrong attribute: \"", args,
              "\" not in provide Log_buffer_Log_I_log");
