...
std::ostream &
Match::to_stream_data(std::ostream &os,
...
std::ostream &
Match::to_stream_sc(std::ostream &os,
                    CompoMe::Serialization_context_export &p_ctx) const {
  os << ",components:{";
...
std::ostream &
Match::to_stream_provide(std::ostream &os,
                         CompoMe::Serialization_context_export &p_ctx) const {
  os << ",provide:{";
...
/////////////////////////////////////////////////////////////////////////////
std::ostream &
Match::to_stream(std::ostream &os,
                 CompoMe::Serialization_context_export &p_ctx) const {
  os << "{";
  os << "addr:" << (void *)this;
  p_ctx.declare(this);
...
std::istream &
Match::from_stream_provide(std::istream &is,
                           CompoMe::Serialization_context_import &p_ctx) {
  char l_c = is.get();
  if (l_c != '{') {
...
std::istream &
Match::from_stream_data(std::istream &is,
                        CompoMe::Serialization_context_import &p_ctx) {
  char l_c = is.get();
  if (l_c != '{') {
    std::cerr << "Wrong start: '" << l_c << "' != '{'";
...
std::istream &
Match::from_stream_sc(std::istream &is,
                      CompoMe::Serialization_context_import &p_ctx) {
  char l_c = is.get();
  if (l_c != '{') {
    std::cerr << "Wrong start: '" << l_c << "' != '{'";
...
std::istream &Match::from_stream(std::istream &is,
                                 CompoMe::Serialization_context_import &p_ctx) {
  Match l_reset;
  *this = l_reset;
  ...
  do {
    ...
    case str2int("type"): {
      ...
      if (t.first != "Match") {
        throw "Wrong Type: "; // + "Match" + " != " + t.first ;
      }
      break;
    }
