#include "Links/CompoMe/Posix/Http_server_map_in/Http_server_map_in.hpp"
...
namespace Posix {
  ...
  Http_server_map_in::Http_server_map_in() : CompoMe::Link(), listening_socket(-1), fds(nullptr), i_fds(0),
                                             buff(nullptr)  {
...
Http_server_map_in::~Http_server_map_in() {}

void Http_server_map_in::step() { Link::step(); 
  ...
  for (int i = 1; i < this->i_fds; i++) {
    ...
    if (this->fds[i].revents & POLLIN) {
      ...
      if(body == "?"){
        std::stringstream ss;
        this->get_map_of_caller_stream()[request.GetPath()]->introspection(ss);
        result_of_call = {true, ss.str()};
      } else {
        result_of_call = CompoMe::Tools::call(this->get_map_of_caller_stream()[request.GetPath()], std::string(body));
      }
...
void Http_server_map_in::connect() { Link::connect(); 
  ...
  void Http_server_map_in::disconnect() { Link::disconnect();   Link::disconnect();
...
CompoMe::String Http_server_map_in::get_addr() const { return this->addr; }

void Http_server_map_in::set_addr(const CompoMe::String p_addr) {
  this->addr = p_addr;
}

CompoMe::String &Http_server_map_in::a_addr() { return this->addr; }
i32 Http_server_map_in::get_port() const { return this->port; }

void Http_server_map_in::set_port(const i32 p_port) { this->port = p_port; }

i32 &Http_server_map_in::a_port() { return this->port; }
ui32 Http_server_map_in::get_max_client() const { return this->max_client; }

void Http_server_map_in::set_max_client(const ui32 p_max_client) {
  this->max_client = p_max_client;
}

ui32 &Http_server_map_in::a_max_client() { return this->max_client; }
ui32 Http_server_map_in::get_max_request_size() const {
  return this->max_request_size;
}

void Http_server_map_in::set_max_request_size(const ui32 p_max_request_size) {
  this->max_request_size = p_max_request_size;
}

ui32 &Http_server_map_in::a_max_request_size() {
  return this->max_request_size;
}
