...
TEST_CASE("call interface function ", "[call][Tools][JSON]") {
  std::map<std::string, CompoMe::Caller_json *> Mcall;

  Interface1_impl i1;
  Interface1_impl i2;
  Interface1_impl i3;

  Mcall["aa"] = i1.get_caller_json();
  Mcall["bb"] = i2.get_caller_json();
  Mcall["cc"] = i3.get_caller_json();
  SECTION("simple") {
    std::string t = GENERATE("aa", "bb", "cc");
